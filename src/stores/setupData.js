// store/setupData.ts
// pinia を利用して global に値の受け渡しを実現する

import { ref, computed } from 'vue'
import { defineStore } from 'pinia'
import { config } from "../config";

export const useSetupDataStore = defineStore('setupData', () => {
  const storedServerSelection = ref(localStorage.getItem("serverSelection") ?? "server1");
  const serverSelection = ref(storedServerSelection == "server1" ? "server1" : "server2");
  const serverUrl = ref(serverSelection == "server1" ? config.serverUrl1 : config.serverUrl2);
  const apiKey1 = ref(localStorage.getItem("apiKey1") ?? "");
  const apiKey2 = ref(localStorage.getItem("apiKey2") ?? "");
  const apiKey = ref(serverSelection == "server1" ? apiKey1 : apiKey2);
  console.log("store init:", { serverSelection }, { serverUrl }, { apiKey });
  const logArray = ref([]); // logを格納するarray
  const notificationData = ref({  path: "",  body: "Body: ",});



  return { storedServerSelection, serverSelection, serverUrl, apiKey1, apiKey2, logArray,notificationData }
